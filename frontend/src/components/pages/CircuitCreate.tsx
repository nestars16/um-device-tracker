...
import { CircuitDTO, RequestResponse } from "@/lib/types";
...
async function saveCircuit(value: CircuitDTO) {
  ...
  const response = await fetch("/api/circuits/create", {
    method: "POST",
    headers: {
...
export function CircuitCreate() {
  const navigation = useNavigation();
  const queryClient = useQueryClient();
  const navigate = useNavigate();

  const [circuit, setCircuit] = React.useState<CircuitDTO>({
    state: "",
    site_name: "",
    ckt_id: "",
    parent: "",
  ...
  const saveCircuitMutation = useMutation({
    ...
    onSuccess: (response: RequestResponse<null>) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["CircuitsAll"] });
    ...
    <Card className="w-full max-w-4xl">
      ...
      <CardFooter>
        <div className="flex justify-end">
          <Button
            onClick={() => saveCircuitMutation.mutate(circuit)}
            disabled={saveCircuitMutation.isPending}
          >
            {saveCircuitMutation.isPending ? (
              <>
